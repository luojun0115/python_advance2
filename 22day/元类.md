...
在大多数编程语言中，{% em color="#fff700" %}类就是一组用来描述如何生成一个对象的代码段{% endem %}。在Python中这一点仍然成立：
...
```
但是，Python中的类还远不止如此。{% em color="#fff700" %}类同样也是一种对象{% endem %}。是的，没错，就是对象。只要你使用关键字class，Python解释器在执行的时候就会创建一个对象。
...
将在内存中创建一个对象，名字就是ObjectCreator。这个对象（类对象ObjectCreator）拥有{% em color="#2ff700" %}创建对象（实例对象）的能力{%endem%}。但是，它的本质仍然是一个对象，于是乎你可以对它做如下的操作：
...
因为{% em color="#fff700" %}类也是对象，你可以在运行时动态的创建它们，就像其他任何对象一样{% endem %}。首先，你可以在函数中创建类，使用class关键字即可。
...
{% em color="#fff700" %}当你使用class关键字时，Python解释器自动创建这个对象。{% endem %}但就和Python中的大多数事情一样，Python仍然提供给你手动处理的方法。
...
{% em color="#fff700" %}type还有一种完全不同的功能，动态的创建类。{% endem %}

{% em color="#2ff700" %}type可以接受一个类的描述作为参数，然后返回一个类{%endem%}。（要知道，根据传入参数的不同，同一个函数拥有两种完全不同的用法是一件很傻的事情，但这在Python中是为了保持向后兼容性）
...
{% em color="#fff700" %}元类就是用来创建类的“东西”。{% endem %}你创建类就是为了创建类的实例对象，不是吗？但是我们已经学习到了Python中的类也是对象。

元类就是用来创建这些类（对象）的，{% em color="#fff700" %}元类就是类的类{% endem %}，你可以这样理解为：
...
```
这是因为函数type实际上是一个元类。{% em color="#2ff700" %}type就是Python在背后用来创建所有类的元类{%endem%}。现在你想知道那为什么type会全部采用小写形式而不是Type呢？好吧，我猜这是为了和str保持一致性，str是用来创建字符串对象的类，而int是用来创建整数对象的类。type就是创建类对象的类。你可以通过检查\_\_class\_\_属性来看到这一点。{% em color="#fff700" %}Python中所有的东西，注意，我是指所有的东西——都是对象。这包括整数、字符串、函数以及类{% endem %}。它们全部都是对象，而且它们都是从一个类创建而来，这个类就是type。
...
{% em color="#2ff700" %}元类的主要目的就是为了当创建类时能够自动地改变类{%endem%}。

假想一个很傻的例子，你决定在你的模块里所有的类的属性都应该是大写形式。有好几种方法可以办到，但其中一种就是通过在模块级别设定\_\_metaclass\_\_。采用这种方法，这个模块中的所有类都会通过这个元类来创建，我们只需要告诉元类把所有的属性都改成大写形式就万事大吉了。

幸运的是，\_\_metaclass\_\_实际上可以被任意调用，它并不需要是一个正式的类。所以，我们这里就先以一个简单的函数作为例子开始。
...
“元类就是深度的魔法，{% em color="#fff700" %}99%的用户应该根本不必为此操心{% endem %}。如果你想搞清楚究竟是否需要用到元类，那么你就不需要它。那些实际用到元类的人都非常清楚地知道他们需要做什么，而且根本不需要解释为什么要用元类。”  —— Python界的领袖 Tim Peters
